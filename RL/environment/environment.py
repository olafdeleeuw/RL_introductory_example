...
class RLEnvironment:
    ...
    # determine if agent is finished
    def update_env_status(self):
        if self.unconnected_houses == 0:
            self.env_status = 1
    ...
    def get_state_id(self):
        ...
        for i in range(len(self.cable_pieces)):
            if self.env_matrix[i, 7] == 0:  # i.e. if this cable piece is not chosen yet
                chosen = 0
            else:
                chosen = 1
    ...
    def get_reward(self):
        ...
        n_moffen = self.env_matrix[:, 5].sum()
        trace_length = self.env_matrix[np.where(self.env_matrix[:, 7] == 1)[0], 4].sum()
    ...
    def verify_path_existence_level(env_matrix, df_nodes, level):
        ...
        for i in range(env_matrix.shape[0]):
            if env_matrix[i, 6] == level and env_matrix[i, 7] == 1:
